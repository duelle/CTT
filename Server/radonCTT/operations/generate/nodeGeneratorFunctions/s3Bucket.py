...
def generateNodeTest(project, testartifact, policyTest, policy, node):
    """
    This is a node generator function for the s3Bucket node type.
...
# ~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+
# AVERAGE RESPONSE TIME POLICY METHODS
# ~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+~+

#------------------------------------------------------|
def generateTestForPolicyAverageResponseTime(project, testartifact, policyTest, policy, node):
    """
    This is a node/policy generator function for the s3Bucket node type and the averageReponseTime policy.
...
def generateTestForPolicyAverageResponseTimeRunnerPython(project, testartifact, policyTest, policy, node):
    """
    This is a node/policy/runner generator function for the s3Bucket node type and the averageReponseTime policy and the python runner.
    The S3Bucket has one more property which we need to go one more generator layer deep, the trigger property.
    If both the upload and download trigger are specified, we need to generate two NodeTests.
    ...
    #Define the node/policy/runner/trigger generator function layer
    functionSwitch = {
        # Upload
        'upload' : generateTestForPolicyAverageResponseTimeRunnerPythonUpload,
        
        # Download
        'download': generateTestForPolicyAverageResponseTimeRunnerPythonDownload,
    }

    for trigger in node['properties']['trigger']:
        nodePolicyRunnerTriggerGeneratorFunction = functionSwitch[trigger]
        nodePolicyRunnerTriggerGeneratorFunction(project, testartifact, policyTest, policy, node)
#------------------------------------------------------|

#------------------------------------------------------|
def generateTestForPolicyAverageResponseTimeRunnerPythonUpload(project, testartifact, policyTest, policy, node):
    """
    This is a >>>TERMINAL<<< generator function for the s3Bucket node type 
    and the averageReponseTime policy and the python runner and the upload trigger.
    ...
    # +++TEMPLATE PARAMETERS+++
    # ARG 1 : REPEATS
    # ARG 2 : SOURCE_FILE_PREFIX
    # ARG 3 : SOURCE_FILE_POSTFIX
    # ARG 4 : BUCKET_NAME
    # ARG 5 : SOURCE_FILE_PATH
    # ARG 6 : RESULT_PATH
    # ARG 7 : UNIQUE_TEST_ID
    # +++++++++++++++++++++++++
    ...
    bucket_name = f"{node['properties']['bucket_name']}"
    ...
    # We need the absolute path to the 
    templatePath = f"{os.getcwd()}{os.path.sep}radonCTT{os.path.sep}templates{os.path.sep}averageResponseTime{os.path.sep}aws{os.path.sep}s3Bucket{os.path.sep}python-upload{os.path.sep}"
    ...
    pythonRunCommand = f"{pythonRunnerPath} {repeats} {source_file_prefix} {source_file_postfix} {bucket_name} {source_file_path} {result_path} {unique_test_id}"
...
#------------------------------------------------------|
def generateTestForPolicyAverageResponseTimeRunnerPythonDownload(project, testartifact, policyTest, policy, node):
    """
    This is a >>>TERMINAL<<< generator function for the s3Bucket node type 
    and the averageReponseTime policy and the python runner and the download trigger.
    It copies the related template and fills it with parameters from the policy and node.
    ...
    templatePath = f"{os.getcwd()}{os.path.sep}radonCTT{os.path.sep}templates{os.path.sep}averageResponseTime{os.path.sep}aws{os.path.sep}s3Bucket{os.path.sep}python-download{os.path.sep}"
